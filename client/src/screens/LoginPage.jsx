...
export default function LoginPage() {
  ...
  return (
    <form className="card flex flex-col text-left relative h-full">
      <Link to="/auth">
        <div className="w-8 h-8 p-1 box-border flex justify-center rounded-full hover:animate-fade">
          <img src={backButton} className="" />
        </div>
      </Link> 
      
      <p className="text-sm mt-8 mb-2">Login</p>
      <p className="text-2.5xl font-bold mb-9">Please enter your details!</p>
      <div>
        <div className="flex space-between">
          <label className="text-sm font-bold mb-3.5" htmlFor="username">
            Username
          </label>
          {errors.username !== "" && (
        ...
        <input
          ...
          className={`w-full leading-4 p-4 pt-5 bg-[#f4f4f4] rounded-lg text-sm mb-4 input ${
            errors.username === "" ? null : "error"
          }`}
        />
      </div>
      <div>
        <div className="flex space-between">
          <label className="text-sm font-bold mb-3.5" htmlFor="password">
            Password
          </label>
          {errors.password !== "" && (
